...
export function JobCard({ job, showApplicationStatus = false }: JobCardProps) {
  const description = cleanJobDescription(job.description);
  const applicationStatus = formatApplicationDisplayStatus(job.applicationDisplayStatus);
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {showApplicationStatus && job.applicationDisplayStatus ? (
          <span
            className={`shrink-0 rounded-full px-3 py-1 text-[12px] font-semibold capitalize leading-[15px] text-white ${getApplicationStatusBadgeClass(job.applicationDisplayStatus)}`}
          >
            {applicationStatus}
          </span>
        ) : null}
