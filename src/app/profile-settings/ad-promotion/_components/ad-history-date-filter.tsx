...
export type AdHistoryDateRange = {
  startDate?: string;
  endDate?: string;
};

type AdHistoryDateFilterProps = {
  startDate?: string;
  endDate?: string;
  onApply: (range: AdHistoryDateRange) => void;
};

export default function AdHistoryDateFilter({
  startDate,
  endDate,
  onApply,
}: AdHistoryDateFilterProps) {
      ...
      {isOpen ? (
        <div className="absolute right-0 top-[calc(100%+8px)] z-20 w-[min(320px,calc(100vw-2rem))] rounded-[16px] border border-[rgba(0,88,100,0.1)] bg-white p-4 shadow-[0_12px_32px_rgba(0,88,100,0.12)]">
          ...
          <p className="mt-1 text-[13px] leading-5 text-[rgba(24,24,24,0.65)]">
            Select a start and end date to filter ad history.
          </p>

          <div className="mt-4 space-y-3">
            <div>
              <label
                htmlFor="ad-history-start-date"
                className="text-[13px] font-[500] text-[rgba(24,24,24,0.8)]"
              >
                Start date
              </label>
              <Input
                id="ad-history-start-date"
                type="date"
            ...
            <div>
              <label
                htmlFor="ad-history-end-date"
                className="text-[13px] font-[500] text-[rgba(24,24,24,0.8)]"
              >
                End date
              </label>
              <Input
                id="ad-history-end-date"
                type="date"
                value={draftEndDate}
