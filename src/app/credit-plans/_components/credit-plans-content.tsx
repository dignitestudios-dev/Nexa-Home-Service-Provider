...
import CustomPackageModal from "./custom-package-modal";
...
export default function CreditPlansContent({
  titleClassName = "text-[32px] font-semibold capitalize leading-[40px] text-black ml-10",
  containerClassName = "mx-auto w-full max-w-[1420px] pb-10",
  checkoutSuccessPath = BILLING_SUCCESS_PATH,
  checkoutCancelPath = BILLING_CANCEL_PATH,
  footer,
}: CreditPlansContentProps) {
  ...
  const startCheckout = (amount: number) => {
    checkoutMutation.mutate({
      amount,
      success_url: getBillingRedirectUrl(checkoutSuccessPath),
      cancel_url: getBillingRedirectUrl(checkoutCancelPath),
    });
  };

  const handleBuyNow = () => {
    if (!selectedPackage) return;
    startCheckout(selectedPackage.price);
  };
  ...
  return (
    <div className={containerClassName}>
      <h1 className={titleClassName}>Credit Plans</h1>
      ...
      <div className="mx-auto flex w-full max-w-[700px] flex-col pt-8">
        ...
        Choose a package that fits your needs. Use credits to unlock jobs and connect directly with homeowners. You can upgrade or purchase more credits anytime.
        </p>
        ...
        <button
          ...
          disabled={true}
          // disabled={checkoutMutation.isPending || !selectedPackage}
